...
- `core/generation/` — `SeededMineGenerator`: deterministic, seed-based generation. Density is an independent hash of `(worldSeed, chunkCoord)` mapped with a cubic ease (`u³`) into `[0.156, 0.35]` (easy-biased mix per selector, no distance ramp). First-touch-safe (excludes the tapped cell and its 8 neighbors) and generates lazy neighbors so adjacency is correct without deferred bookkeeping; `ensureNeighborsGenerated` expands the neighbor ring before reveals so border numbers stay stable.
