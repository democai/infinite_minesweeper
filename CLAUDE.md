...
- `core/generation/` — `SeededMineGenerator`: deterministic, seed-based generation. Density is an independent hash of `(worldSeed, chunkCoord)` mapped with a cubic ease (`u³`) into `[0.156, 0.35]` (easy-biased mix per selector, no distance ramp). First-touch-safe (excludes the tapped cell and its 8 neighbors) and generates lazy neighbors so adjacency is correct without deferred bookkeeping; `ensureNeighborsGenerated` expands the neighbor ring before reveals so border numbers stay stable. Reset rerolls keep the selector's 28-cell mine perimeter and replace only its 6×6 interior.
...
Revealing a mine locks its chunk (status `LOCKED`, input disabled, red overlay) rather than ending the game. A locked chunk resolves once all 8 neighbor chunks are solved with every safe cell revealed and every mine flagged (a locked neighbor never counts as solved). **First resolution is soft**: the mine is removed, the cell becomes a number, adjacency is recomputed, and `everSurrounded` becomes `true`. **Any later mine hit in a chunk with `everSurrounded == true` is a hard wipe**: all 64 cells go back to hidden, the selector keeps its mine perimeter, its 6×6 interior is rerolled deterministically, `everSurrounded` resets to `false`, and `selectorsWiped` increments. There is exactly one soft save per chunk, ever. Because perimeter mines stay fixed during a wipe, valid revealed clues in neighboring selectors do not change.
